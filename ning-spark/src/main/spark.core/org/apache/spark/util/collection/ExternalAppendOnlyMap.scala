...
class ExternalAppendOnlyMap[K, V, C](
   ...
   */
  private val serializerBatchSize = sparkConf.getLong("spark.shuffle.spill.batchSize", 10000) //批处理记录数量 1W
  ...
  private val fileBufferSize =
    sparkConf.getSizeAsKb("spark.shuffle.file.buffer", "32k").toInt * 1024 //文件缓冲大小32K
  ...
  private val keyComparator = new HashComparator[K] //key比较器
  private val ser = serializer.newInstance() //序列化

  /**
   * Number of files this map has spilled so far.
   * Exposed for testing.
   */
  private[collection] def numSpills: Int = spilledMaps.size //溢出了多少个MAP
  ...
  def insertAll(entries: Iterator[Product2[K, V]]): Unit = {
    ...
    val update: (Boolean, C) => C = (hadVal, oldVal) => {//更新函数
      if (hadVal) mergeValue(oldVal, curEntry._2) else createCombiner(curEntry._2)
    }

    while (entries.hasNext) {//下一个元素
      curEntry = entries.next()
      val estimatedSize = currentMap.estimateSize()//评估当前MAP的大小
      if (estimatedSize > _peakMemoryUsedBytes) { //_peakMemoryUsedBytes记录内存峰值
        _peakMemoryUsedBytes = estimatedSize
      }
      if (maybeSpill(currentMap, estimatedSize)) {//判断是否往磁盘写出
        currentMap = new SizeTrackingAppendOnlyMap[K, C]//写出之后重新实例化集合
      }
      currentMap.changeValue(curEntry._1, update)//更新集合
      addElementsRead()//记录读取的元素数量
    }
  }
