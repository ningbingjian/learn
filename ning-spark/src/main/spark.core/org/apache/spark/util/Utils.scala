...
private[spark] object Utils extends Logging {
   ...
   * exception from the original `out.write` call.
    * 执行一个块的代码和一个finally块代码。但是如果在finally块代码的时候出现异常，不会把原始的异常吃掉
    * 这个主要解决`fihnally{out.close()}` 这种代码块，在那些需要调用清理输出流的地方，但是如果异常发生在out.write代码，
    * 可能out已经出现问题了，那么out.close肯定也会出现问题。这样的话可能会异常原始异常
    *
   */
