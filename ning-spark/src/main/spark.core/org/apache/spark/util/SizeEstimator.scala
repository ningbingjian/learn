...
object SizeEstimator extends Logging {
  ...
  private var objectSize = 8 //一个object对象的最小大小
   ...
   * to visit.
   */
  //正在进行规模大小评估的状态,包含stack保存评估对象的大小,IdentityHashMap确保不是同一个对象
  ...
  private def visitSingleObject(obj: AnyRef, state: SearchState) {
    val cls = obj.getClass//class
    if (cls.isArray) {//如果是数组
      ...
      obj match {//如果是某些已经预测的对象 只需要累加进来即可
        case s: KnownSizeEstimation =>
          state.size += s.estimatedSize
        //
        case _ =>
          val classInfo = getClassInfo(cls)
          state.size += alignSize(classInfo.shellSize) //计算大小
   ...
   */
  private def getClassInfo(cls: Class[_]): ClassInfo = { //获取类的信息，根据class
    // Check whether we've already cached a ClassInfo for this class
    val info = classInfos.get(cls) //类信息
    if (info != null) {
      return info
    }

    val parent = getClassInfo(cls.getSuperclass) //父类信息
    var shellSize = parent.shellSize //父类大小
    var pointerFields = parent.pointerFields //父类的引用的成员变量 默认还没实现
    val sizeCount = Array.fill(fieldSizes.max + 1)(0) //数组大小 ，默认初始化为0

    // iterate through the fields of this class and gather information.
    for (field <- cls.getDeclaredFields) {//遍历所有的成员
      if (!Modifier.isStatic(field.getModifiers)) {//是否是静态成员
        val fieldClass = field.getType//成员类型
        if (fieldClass.isPrimitive) {//8种主要类型
          sizeCount(primitiveSize(fieldClass)) += 1//更新数组对应类型 sizeCount解释：每个位置表示 8,4,2,1的倍数  因为JVM对象的大小都是这几个数
        } else {
          field.setAccessible(true) // Enable future get()'s on this field
          sizeCount(pointerSize) += 1 //引用类型 引用本身 4个字节
