...
class SparkContext(config: SparkConf) extends Logging with ExecutorAllocationClient {
  ...
  try {
    ...
    _taskScheduler = ts
    //_dagScheduler持有sc，也就是sc持有的成员它都能访问到
  ...
  private[spark] def unpersistRDD(rddId: Int, blocking: Boolean = true) {
    env.blockManager.master.removeRdd(rddId, blocking) //本地删除RDD的映射关系
    persistentRdds.remove(rddId) //删除RDD
  ...
  private def createTaskScheduler(
    ...
    master match {
      ...
      case SPARK_REGEX(sparkUrl) =>
        //目前实现只有TaskSchedulerImpl
        val scheduler = new TaskSchedulerImpl(sc)
        //master地址
        val masterUrls = sparkUrl.split(",").map("spark://" + _)

        /**
          *   SparkDeploySchedulerBackend继承CoarseGrainedSchedulerBackend
          *   并且持有scheduler
          */
