    ...
    try {
      val blockFetchStarter = new RetryingBlockFetcher.BlockFetchStarter {
        override def createAndStart(blockIds: Array[String], listener: BlockFetchingListener) {
          val client = clientFactory.createClient(host, port)//创建调用远程的客户端
          new OneForOneBlockFetcher(client, appId, execId, blockIds.toArray, listener).start()//获取数据
        }
      }
      //重试3次
      val maxRetries = transportConf.maxIORetries()
      if (maxRetries > 0) {//作者怀疑这里有bug，把maxRetries==0放到else部分执行了，如果稳定的话作者会加上maxRetries >=0得条件
