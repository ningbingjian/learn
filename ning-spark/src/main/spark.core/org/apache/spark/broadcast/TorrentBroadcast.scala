...
private[spark] class TorrentBroadcast[T: ClassTag](obj: T, id: Long)
  ...
  private def writeBlocks(value: T): Int = {
    ...
    //在driver端保存一份广播变量 这样driver端想用的时候就可以直接
  ...
  //从driver获取其他executor获取广播变量
  private def readBlocks(): Array[ByteBuffer] = {
    // Fetch chunks of data. Note that all these chunks are stored in the BlockManager and reported
    // to the driver, so other executors can pull these chunks from this executor as well.
    //获取数据的chunk,注意所有这些chunk都是保存在blockmanager和报告给driver的。所以其他executor也可以从当前的executor拉取数据。
  ...
  private def readBroadcastBlock(): T = Utils.tryOrIOException {
    TorrentBroadcast.synchronized { //有可能多个线程对广播变量的操作 所以需要同步的方式
      setConf(SparkEnv.get.conf)
      //先从本地获取
      SparkEnv.get.blockManager.getLocal(broadcastId).map(_.data.next()) match {
        ...
        case None => //本地没有
          ...
          // need to re-fetch it.
          //在当前的executor保存一份广播变量 下次获取的时候可以直接从本地获取
