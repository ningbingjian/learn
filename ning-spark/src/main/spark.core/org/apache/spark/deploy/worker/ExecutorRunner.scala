...
private[deploy] class ExecutorRunner(
  ...
  private[worker] def kill() {
    if (workerThread != null) {
      ...
      workerThread.interrupt()//调用中断来杀死workerThread线程
      workerThread = null
      state = ExecutorState.KILLED//修改状态
      try {
        ShutdownHookManager.removeShutdownHook(shutdownHook)//移除在启动executor进程设置的回调钩子
      } catch {
  ...
  private def fetchAndRunExecutor() {
    try {
      ...
      //设置进程工作目录 SPARK_HOME/work/execId
      builder.directory(executorDir)
      //executor的目录 也就是APP的本地目录默认是/tmp/executor-UUID字符
