...
private[deploy] class Worker(
  ...
  override def receive: PartialFunction[Any, Unit] = synchronized {
    ...
    case LaunchExecutor(masterUrl, appId, execId, appDesc, cores_, memory_) =>
      if (masterUrl != activeMasterUrl) {
        ...
        try {
          ...
          //创建executor的工作目录,默认在SPARK_HOME/work/execId
          val executorDir = new File(workDir, appId + "/" + execId)
          if (!executorDir.mkdirs()) {//没有权限创建目录 失败
        ...
        //创建app运行时的本地工作目录，完成后会被worker自动删除,默认如果没有配置的话就在/tmp/executor-UUID字符
          ...
          val manager = new ExecutorRunner(
            appId,//应用ID
            execId,//executorId
            appDesc.copy(command = Worker.maybeUpdateSSLSettings(appDesc.command, conf)),//app描述信息
            cores_,//core数量
            memory_,//内存
            self,//当前对象
            workerId,//workerId
            host,//主机
            webUi.boundPort,//web端口
            publicAddress,//地址
            sparkHome,//spark家目录
            executorDir,//executor工作目录
            workerUri,//workerURL
            conf,//配置
            appLocalDirs, //app运行时工作目录
            ExecutorState.RUNNING)//运行状态
          executors(appId + "/" + execId) = manager//保存映射关系
          manager.start()//在worker启动executor的进程
          coresUsed += cores_ //记录core使用
          memoryUsed += memory_ //记录内存使用
          sendToMaster(ExecutorStateChanged(appId, execId, manager.state, None, None))//发送给master executor变化
        } catch {
          case e: Exception => {
    ...
    case KillExecutor(masterUrl, appId, execId) => //收到杀死executor的消息
      if (masterUrl != activeMasterUrl) {
        logWarning("Invalid Master (" + masterUrl + ") attempted to launch executor " + execId)
      } else {
        val fullId = appId + "/" + execId //executor的定位是appId/execId
        executors.get(fullId) match {
          case Some(executor) =>
            logInfo("Asked to kill executor " + fullId)
            executor.kill()//调用杀死该executor
          case None =>
