...
private[spark] class AppClient(
  ...
  private class ClientEndpoint(override val rpcEnv: RpcEnv) extends ThreadSafeRpcEndpoint
    ...
    override def onStart(): Unit = {
      try {
        //向master注册应用，重试一次即可，大部分时候都能注册成功
      ...
      registrationRetryTimer.set(registrationRetryThread.scheduleAtFixedRate(new Runnable {
        override def run(): Unit = {
          Utils.tryOrExit {
            ...
            } else if (nthRetry >= REGISTRATION_RETRIES) {//默认重试3次
    ...
    override def receive: PartialFunction[Any, Unit] = {
      case RegisteredApplication(appId_, masterRef) =>
        ...
        // changing.
        //设置应用ID 应用ID是master生成的
        appId.set(appId_)
        //设置注册标记
        registered.set(true)
        //master的地址引用
        master = Some(masterRef)
        //监听器链接 这里listener实际上是SparkDeploySchedulerBackend,
        // 这里会调用registrationBarrier.release释放注册时期的阻塞
      ...
      case ExecutorUpdated(id, state, message, exitStatus) =>
        ...
        if (ExecutorState.isFinished(state)) {//executor退出
          //通知Applicationlistener移除executor 这里的listener实际上是SparkDeploySchedulerBackend
