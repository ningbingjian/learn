...
private[spark] class MemoryStore(blockManager: BlockManager, memoryManager: MemoryManager)
  ...
  // Initial memory to request before unrolling any block 在展开任何数据块之前初始化内存请求 1M
   ...
   * This does not include memory used for unrolling.
    * 存储内存的总量。字节表示。主要用来缓存数据块。 没有包含未展开的内存
   */
  ...
  override def putBytes(blockId: BlockId, _bytes: ByteBuffer, level: StorageLevel): PutResult = {
    ...
    //拷贝一份  原来的数据可以随时使用在别的地方
    val bytes = _bytes.duplicate()
    bytes.rewind()
    if (level.deserialized) {//是否反序列化 有可能字节已经是经过压缩和序列化之后的字节，需要反序列化
      val values = blockManager.dataDeserialize(blockId, bytes)//反序列化的值
  ...
  override def putArray(
    ...
    if (level.deserialized) {//序列化
      val sizeEstimate = SizeEstimator.estimate(values.asInstanceOf[AnyRef])//评估大小
      tryToPut(blockId, values, sizeEstimate, deserialized = true, droppedBlocks)//尝试放入内存
      PutResult(sizeEstimate, Left(values.iterator), droppedBlocks)//结果
    } else {
  ...
  private[storage] def putIterator(
    ...
    unrolledValues match {
      case Left(arrayValues) => //内存足够  所有的数据块都展开了
        // Values are fully unrolled in memory, so store them as an array
        val res = putArray(blockId, arrayValues, level, returnValues) //作为数组存储
  ...
  override def remove(blockId: BlockId): Boolean = memoryManager.synchronized {
    val entry = entries.synchronized { entries.remove(blockId) } //linkeHashMap中删除对应数据
    if (entry != null) {
      memoryManager.releaseStorageMemory(entry.size) //释放内存
  ...
  def unrollSafely(
    ...
    // Number of elements unrolled so far 目前为止展开的元素数量
    var elementsUnrolled = 0
    // Whether there is still enough memory for us to continue unrolling this block 是否有足够的内存继续展开该数据块
    var keepUnrolling = true
    // Initial per-task memory to request for unrolling blocks (bytes). Exposed for testing. 初始化每个任务的内存请求对于展开的数据块
    val initialMemoryThreshold = unrollMemoryThreshold
    // How often to check whether we need to request more memory 多久检测一次来查看是否需要更多的内存
    val memoryCheckPeriod = 16
    // Memory currently reserved by this task for this particular unrolling operation  当前任务目前为这个特定的展开操作保留的内存
    var memoryThreshold = initialMemoryThreshold
    // Memory to request as a multiple of current vector size 内存增长因子 1.5
    val memoryGrowthFactor = 1.5
    // Keep track of pending unroll memory reserved by this method. 跟踪这个方法保留的未展开的内存
    var pendingMemoryReserved = 0L
    // Underlying vector for unrolling the block 展开数据块的底层向量 具有追踪大小的功能
    var vector = new SizeTrackingVector[Any]

    // Request enough memory to begin unrolling 没有展开数据块之前请求足够的内存
    keepUnrolling = reserveUnrollMemoryForThisTask(blockId, initialMemoryThreshold, droppedBlocks)

    if (!keepUnrolling) {//内存不足
    ...
    try {
      while (values.hasNext && keepUnrolling) {//下一个
        vector += values.next()//添加
        if (elementsUnrolled % memoryCheckPeriod == 0) { //每展开16个k-v就检查内存使用情况
          // If our vector's size has exceeded the threshold, request more memory
          val currentSize = vector.estimateSize()
          if (currentSize >= memoryThreshold) {//超出内存阀值  请求足够内存
            val amountToRequest = (currentSize * memoryGrowthFactor - memoryThreshold).toLong //根据增长因子和当前内存情况计算需要申请的内存
            keepUnrolling = reserveUnrollMemoryForThisTask( //申请内存
      ...
      if (keepUnrolling) { //成功展开所有内存
        // We successfully unrolled the entirety of this block
        Left(vector.toArray)
      } else {
        // We ran out of space while unrolling the values for this block 展开了所欲内容的同时耗尽了所有内存空间
      ...
      if (keepUnrolling) { //如果我们返回一个数组，在这里的返回值将被缓存在后面'tryToPut'方法。在这个case中，我们必须释放内存
      ...
      } else { //否则 如果我们返回一个迭代器，我们可以当任务完成后只释放这些没有展开的内存，因为我们不知道迭代器什么时候被消费。
  ...
  private def tryToPut(
      ...
      value: Any,//值
      size: Long,//大小
      deserialized: Boolean,//是否需反序列化
      droppedBlocks: mutable.Buffer[(BlockId, BlockStatus)]): Boolean = {
    tryToPut(blockId, () => value, size, deserialized, droppedBlocks)//尝试放入内存
  }
  ...
  private def tryToPut(
    ...
    memoryManager.synchronized {
      ...
      //如果之前没有成功展开块数据，那么待展开的内存应该是非0的。这个就是我们在未展开块处理的时候保留的总量。这种情况下我们可以仅仅重用这些空间来缓存我们的数据块。
      //memoryManager 在这里的同步保证释放和请求是原子操作。这些都依赖于假设所有的内存请求都是在一个锁上进行同步的
      releasePendingUnrollMemoryForThisTask()
      val enoughMemory = memoryManager.acquireStorageMemory(blockId, size, droppedBlocks)//判断内存是否足够
      if (enoughMemory) {
        // We acquired enough memory for the block, so go ahead and put it
        val entry = new MemoryEntry(value(), size, deserialized)//存放数据
        entries.synchronized {//真正的数据存放
        ...
        //告诉blockmanager不能把数据块存都存放到内存。如果可以允许磁盘存储的话可以存放到磁盘s
        lazy val data = if (deserialized) {
          Left(value().asInstanceOf[Array[Any]])
        } else {
          Right(value().asInstanceOf[ByteBuffer].duplicate())
        }
        //从内存中移除 如果允许磁盘存储的话 使用磁盘存储
