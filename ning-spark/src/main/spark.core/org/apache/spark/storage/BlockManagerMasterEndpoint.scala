...
//接收到更新块的消息
  ...
  private def removeRdd(rddId: Int): Future[Seq[Int]] = {
    ...
    //首先移除RDD元数据，然后异步的从slave节点移除数据块
    //找到和RDD有关的block块，从blockLocations和blockManagerInfo移除数据块。blockManagerInfo 负责追踪数据块
    val blocks = blockLocations.asScala.keys.flatMap(_.asRDDId).filter(_.rddId == rddId)
    blocks.foreach { blockId => //获取数据块位置信息
      val bms: mutable.HashSet[BlockManagerId] = blockLocations.get(blockId)
      bms.foreach(bm => blockManagerInfo.get(bm).foreach(_.removeBlock(blockId))) //移除block信息
      blockLocations.remove(blockId)//删除块的位置信息
    }

    // Ask the slaves to remove the RDD, and put the result in a sequence of Futures.
    // The dispatcher is used as an implicit argument into the Future sequence construction.
    //向所有保存有block的slave节点发送删除RDD的消息
    val removeMsg = RemoveRdd(rddId)
    Future.sequence(
      blockManagerInfo.values.map { bm =>
        bm.slaveEndpoint.ask[Int](removeMsg)//  blockmanager.slaveEndpoint
      }.toSeq
    )
  }
...
//如果已经不包含该数据块了
    if (!blockManagerInfo.contains(blockManagerId)) {
      if (blockManagerId.isDriver && !isLocal) {//driver并且不是本地模式
        // We intentionally do not register the master (except in local mode),
        // so we should not indicate failure.
        //返回true 表示不需要重新注册了该block
        return true
      } else {//其他的更新操作 需要重新注册该block
        return false
      }
    }

    if (blockId == null) { //如果是空的话
      blockManagerInfo(blockManagerId).updateLastSeenMs()
      return true
    }
    //更新数据块信息
