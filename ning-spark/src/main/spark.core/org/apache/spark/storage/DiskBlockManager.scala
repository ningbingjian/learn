...
private[spark] class DiskBlockManager(blockManager: BlockManager, conf: SparkConf)
  ...
  def getFile(filename: String): File = {
    ...
    //子目录ID默认就是 hash值得除以64取余  默认每个目录只能由64个子目录
    ...
    val subDir = subDirs(dirId).synchronized {
      ...
      if (old != null) {
        ...
        //默认路径名就是/tmp/blockmgr-UUID/subDirId
        subDirs(dirId)(subDirId) = newDir
        newDir
      }
    }
    //对于hash shuffleMaptask得输出，默认的目录大概就是/tmp/blockmgr-UUID/subDirId/"shuffle_" + shuffleId + "_" + mapId + "_" + reduceId
