...
class DAGScheduler(
  ...
  private[scheduler] def handleTaskCompletion(event: CompletionEvent) {
    val task = event.task//具体任务
    val stageId = task.stageId//任务所在的stage
    val taskType = Utils.getFormattedClassName(task)//任务类型
    //输出协调器处理任务完成
    ...
    if (event.reason != Success) {//失败任务处理
      val attemptId = task.stageAttemptId
      listenerBus.post(SparkListenerTaskEnd(stageId, attemptId, taskType, event.reason,
        event.taskInfo, event.taskMetrics))//给监听总线发送失败内容 可以用来告诉页面展示
    }

    if (!stageIdToStage.contains(task.stageId)) {//stage被取消了
    ...
    event.reason match {
      case Success =>
        listenerBus.post(SparkListenerTaskEnd(stageId, stage.latestInfo.attemptId, taskType,
          event.reason, event.taskInfo, event.taskMetrics))//成功任务
        ...
        task match {//任务结果
          case rt: ResultTask[_, _] =>
            ...
            resultStage.activeJob match {//判断job
              case Some(job) =>
                if (!job.finished(rt.outputId)) {
                  updateAccumulators(event)//更新累加器
                  job.finished(rt.outputId) = true
                  job.numFinished += 1
                  // If the whole job has finished, remove it
                  if (job.numFinished == job.numPartitions) {//job完成
                    markStageAsFinished(resultStage)
                    cleanupStateForJobAndIndependentStages(job)
                    listenerBus.post(
                      SparkListenerJobEnd(job.jobId, clock.getTimeMillis(), JobSucceeded))//job完成通知监听总线
                  }

                  // taskSucceeded runs some user code that might throw an exception. Make sure
                  // we are resilient against that.
                  try {
                    job.listener.taskSucceeded(rt.outputId, event.result)//job监听器调用任务完成
                  } catch {
                    case e: Exception =>
        ...
        //shuffle任务的处理
          case smt: ShuffleMapTask =>
            ...
            val status = event.result.asInstanceOf[MapStatus]//mapstatus保存了输出的BlockManagerId
            val execId = status.location.executorId//任务执行的executorId
            logDebug("ShuffleMapTask finished on " + execId)
            if (failedEpoch.contains(execId) && smt.epoch <= failedEpoch(execId)) {
              logInfo(s"Ignoring possibly bogus $smt completion from executor $execId")
            } else {
              shuffleStage.addOutputLoc(smt.partitionId, status)//添加分区对应的status
            }

            //stage所有的任务完成了
            if (runningStages.contains(shuffleStage) && shuffleStage.pendingPartitions.isEmpty) {
              markStageAsFinished(shuffleStage)//标记该stage已经完成
              ...
              //向MapOutputTrackerMaster注册shuffle的输出位置
              ...
              if (!shuffleStage.isAvailable) {//输出的位置信息数量和分区数量的位置信息不一样 分区丢失
                // Some tasks had failed; let's resubmit this shuffleStage
                // TODO: Lower-level scheduler should also deal with this
                logInfo("Resubmitting " + shuffleStage + " (" + shuffleStage.name +
                  ") because some of its tasks had failed: " +
                  shuffleStage.findMissingPartitions().mkString(", "))
                submitStage(shuffleStage)//重新提交该stage重新计算
              } else {
                // Mark any map-stage jobs waiting on this stage as finished
                if (shuffleStage.mapStageJobs.nonEmpty) {
                  val stats = mapOutputTracker.getStatistics(shuffleStage.shuffleDep)
                  for (job <- shuffleStage.mapStageJobs) {//都完成了
