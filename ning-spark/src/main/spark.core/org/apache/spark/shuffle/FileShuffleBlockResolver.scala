...
private[spark] class FileShuffleBlockResolver(conf: SparkConf)
   ...
   */
  //返回一组shufflewriter 所以在map端会输出M*R个文件
    ...
    new ShuffleWriterGroup {
      ...
      val writers: Array[DiskBlockObjectWriter] = {
        Array.tabulate[DiskBlockObjectWriter](numReducers) { bucketId =>
          val blockId = ShuffleBlockId(shuffleId, mapId, bucketId)//"shuffle_" + shuffleId + "_" + mapId + "_" + reduceId 构成当前记录输出的文件名ID
          val blockFile = blockManager.diskBlockManager.getFile(blockId)//
          val tmp = Utils.tempFileWith(blockFile)//"shuffle_" + shuffleId + "_" + mapId + "_" + reduceId+"."+UUID构成文件名
          //文件写出的writer
