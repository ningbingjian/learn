...
private[memory] class StorageMemoryPool(lock: Object) extends MemoryPool(lock) with Logging {
  ...
  def acquireMemory(
    ...
    val numBytesToFree = math.max(0, numBytes - memoryFree) //memoryFree本身剩余内存  numBytes 当前申请内存大小
    acquireMemory(blockId, numBytes, numBytesToFree, evictedBlocks) //申请内存
  }
  ...
  def acquireMemory(
    ...
    if (numBytesToFree > 0) { //numBytesToFree 需要释放的内存大小
      memoryStore.evictBlocksToFreeSpace(Some(blockId), numBytesToFree, evictedBlocks) //删除数据块来达到内存足够
      // Register evicted blocks, if any, with the active task metrics 注册删除的块，如果有得话，使用当前激活的任务度量
    ...
    val enoughMemory = numBytesToAcquire <= memoryFree //判断是否有足够的内存
    if (enoughMemory) { //有足够的内存
