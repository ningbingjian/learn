...
private[spark] abstract class MapOutputTracker(conf: SparkConf) extends Logging {
   ...
   *         describing the shuffle blocks that are stored at that block manager.
   */
  //根据shuffleId,start ,end。在executor端调用来获取每个shuffle块的server uri 和输出大小
   ...
   */
  //根据shuffleId获取MapStatus 注意：客户端读取的时候必须使用同步，因为driver端可能会更改内容
  //以后可能会移除这个限制
  private def getStatuses(shuffleId: Int): Array[MapStatus] = {
    ...
    if (statuses == null) {
      ...
      fetching.synchronized {
        ...
        if (fetchedStatuses == null) {//如果为空
          // We have to do the fetch, get others to wait for us.
          fetching += shuffleId
        }
      }

      if (fetchedStatuses == null) {//如果没有输出状态的信息 需要从driver端获取
        // We won the race to fetch the statuses; do so
        logInfo("Doing the fetch; tracker endpoint = " + trackerEndpoint)
        // This try-finally prevents hangs due to timeouts:
        try {
          //获取序列化后的mapstatus 发送给driver端的MapOutputTrackerMaster进行获取
...
private[spark] class MapOutputTrackerMaster(conf: SparkConf)
   ...
   */
    //executor在shuffle任务完成后都会向driver注册输出块的信息 shuffleId --> Array[MapStatus]
  ...
  //通过shuffleId获取 MapStatus得到的数据块可能是分布于多个节点上
