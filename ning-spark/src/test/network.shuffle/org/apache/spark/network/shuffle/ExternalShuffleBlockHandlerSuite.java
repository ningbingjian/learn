...
public class ExternalShuffleBlockHandlerSuite {
  ...
  //验证调用blockResolver的.registerExecutor("app0", "exec1", config)方法一次
    verify(blockResolver, times(1)).registerExecutor("app0", "exec1", config);
    //验证调用了一次callback的onSuccess(any(ByteBuffer.class)方法
    verify(callback, times(1)).onSuccess(any(ByteBuffer.class));
    //验证调用了一次callback的onFailure(any(Throwable.class)方法
  ...
  public void testOpenShuffleBlocks() {
    ...
    //当调用blockResolver.getBlockData("app0", "exec1", "b0")方法返回block0Marker
    when(blockResolver.getBlockData("app0", "exec1", "b0")).thenReturn(block0Marker);
    //当调用blockResolver.getBlockData("app0", "exec1", "b1")方法返回block1Marker
    ...
    //验证调用了一次blockResolver的getBlockData("app0", "exec1", "b0")方法
    verify(blockResolver, times(1)).getBlockData("app0", "exec1", "b0");
    //验证调用了一次blockResolver的getBlockData("app0", "exec1", "b1")方法
    verify(blockResolver, times(1)).getBlockData("app0", "exec1", "b1");
    //捕获ByteBuffer.class变量
    ArgumentCaptor<ByteBuffer> response = ArgumentCaptor.forClass(ByteBuffer.class);
    //验证调用了一次callback的onSuccess(response.capture())
    verify(callback, times(1)).onSuccess(response.capture());
    //验证没有调用callback的onFailure((Throwable) any())
    ...
    //验证调用了一次streamManager的registerStream方法,stream捕获传递给第二个变量的值，
