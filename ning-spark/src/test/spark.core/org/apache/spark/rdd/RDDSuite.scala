...
class RDDSuite extends SparkFunSuite with SharedSparkContext {
  ...
  test("coalesced RDDs with locality") {
    //
    /**sc.makeRDD生辰了new ParallelCollectionRDD[T](this, seq.map(_._1), seq.size, indexToPrefs)
      * this -->sc
      *seq.map(_._1) -->
      * (1,List(a, c))
        (2,List(a, b, c))
  ...
  test("zipped RDDs") {
    val nums = sc.makeRDD(Array(1, 2, 3, 4), 2)
    //两个RDD配对，必须要有相同 分区数量，分区内元素个数也必须相同
  ...
  test("partition pruning") {
    ...
    //分区大于8的留下，分区是从0开始的，大于8意味着只剩下第10个分区
  ...
  test("top with custom ordering") {
    ...
    val rdd = sc.makeRDD(words, 1)
    val topK = rdd.top(4)
    assert(topK.size === 4)
    println(topK.toList)
    println(topK.sorted.toList)
    //assert(topK.sorted === Array("a", "b"))
  }

  //takeOrdered 方法 预定义排序 利用 Ordering的隐式转换
...
//takeOrdered 使用预定义排序
  ...
  //在transformations操作内部不能包含RDD的任何动作
