...
import org.apache.spark.{HashPartitioner, Partitioner, SharedSparkContext, SparkFunSuite}
...
class PairRDDFunctionsSuite extends SparkFunSuite with SharedSparkContext {
  ...
  test("subtract") {
    ...
    val rdd3 = new SubtractedRDD(rdd1,rdd2,rdd1.partitioner.getOrElse(new HashPartitioner(rdd1.partitions.length)))
