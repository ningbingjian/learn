...
public class TransportConf {
   ...
   *接收缓冲设置规则 是 延迟*带宽
   * 例如：延迟是1ms 带宽是10Gbps
   * 10Gbps = 10 * 1000 * 1000 bit = 1.25MB
   * 那么缓冲区大小应该设置大概是1.25M= 1.25MB * 1ms
   *
   */
  ...
  读取文件的时候的最小块大小设置，如果小于这个值，直接全部读取到内存，大于这个值，使用正常的IO操作。
 这个可以防止spark从内存中映射比较小的块。
   一般情况，当数据块接近或者低于操作系统的页面大小的时候，内存映射具有较高的开销？
   spark.storage.memoryMapThreshold 存储的内存映射阀值 默认2M
