 ...
 * should push this into Netty so the native Epoll transport can support this feature.
 一个FileRegion的实现，仅仅在传输时创建文件描述符。这个不能使用Epoll模型因为没有本地的支持
 这个类大部分是拷贝Netty的DefaultFileRegion实现。将来我们应该把这个实现提交到netty，可以让本地Epoll传输也可以支持这个特性
...
public final class LazyFileRegion extends AbstractReferenceCounted implements FileRegion {
   ...
   * @param position startServer position for the transfer.
