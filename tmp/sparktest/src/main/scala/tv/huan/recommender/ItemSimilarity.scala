...
object ItemSimilarity {
  ...
  /*def pearsonSimilarity(user_rdd:RDD[ItemPref]):RDD[ItemSimi] = {
    // 0 数据做准备 (user,(item,pref))
    val u_ip_rdd = user_rdd.map{
      case (userid,itemid,pref) => (userid,(itemid,pref))
    }
    // 1 (用户,物品,评分) 笛卡尔积 (用户,物品,评分) => （物品1,物品2,评分1,评分2）组合
    val u_ip_join_rdd = u_ip_rdd join u_ip_rdd
    val i_i_p_p_rdd = u_ip_join_rdd.map {
      case (userid,((item1,pref1),(item2,pref2))) => ((item1,item2),(pref1,pref2))
    }
    // 2 （物品1,物品2,评分1,评分2）组合 => （物品1,物品2,评分1*评分2） 组合 并累加
    null
  }*/
